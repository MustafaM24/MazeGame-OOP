...
SDL_Rect warriorMover={430, 0, 100, 100};
...
void moveSouth()
{ 
    if (warriorMover.y+yJump < SCREEN_HEIGHT)
	{
...
void moveEast()
{
	if (warriorMover.x+xJump < SCREEN_WIDTH)
	{
		warriorMover.x+=xJump;
	}
}
void moveWest()
{
	if (warriorMover.x-xJump > 0)
	{
		warriorMover.x-=xJump;
	}
}
